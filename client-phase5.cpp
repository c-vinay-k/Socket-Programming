...
void server_thread1(int* nb_port,int* nb_id)
...
void client_thread3(string filename, int nport)
{
    ...
    int string_size;
        ...
        if (filename != "x")
        {
            ...
            while(sz <=0){
                sz = recv(send_sock, c, sizeof(c), 0);
            }
...
////////////////////////////////////// Reading input from text file is done //////////////
    ...
    std::thread server1(server_thread1,&nb_port[0],&nb_id[0]);
    //cout<<"Hi"<<endl;
    ...
    server2.join();
    for(int i=0;i<n_f;i++){
        if(a[2*i]>0)cout<<f[i]<<" " <<a[2*i]<<endl;
    }
